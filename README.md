...
3. **Dataset Wide Interpretations**:
  - Shows the average RSS value of each perceptual feature w.r.t a specific class
  - Interprets which perceptual features are more influential for the classification of a repsective class
  - <b> For more detailed description of Dataset Wide Interpretations provided by an EPU-CNN model please refer to the EPU-CNN [paper](https://www.nature.com/articles/s41598-023-38459-1)</b>
...
#### Dataset-Wide Interpretations
1. Dataset-Wide interpretation for the FAKE class
![Dataset-Wide Interpretation FAKE class](./assets/FAKE_average_rss.png)

2. Dataset-Wide Interpretation for the REAL class
![Dataset-Wide Interpretation REAL class](./assets/REAL_average_rss.png)

*Figure: Dataset-Wide Interpretations, visualizing the average perceptual feature contribution per class given a labeled test or validation set.*

> RSS and PRM interpretation visualizations are automatically generated when you run:
> ```bash
...
> Dataset-Wide interpretation visualizations generated when you run the `eval.py` script with the `--dataset_wide_interpretations` flag:
> ```bash
> python scripts/eval.py --model_path <your_model_path> --test_data <your_test_data_path> --dataset_wide_interpretations
> ```
> The results are saved in the `eval_results/` directory.
